...
<title>修改链接信息 - 链接管理 - H-ui.admin v3.1</title>
<meta name="keywords" content="H-ui.admin v3.1,H-ui网站后台模版,后台模版下载,后台管理系统模版,HTML后台模版下载">
<meta name="description" content="H-ui.admin v3.1，是一款由国人开发的轻量级扁平化网站后台模板，完全免费开源的网站后台管理系统模版，适合中小型CMS后台系统。">
</head>
<body>
	<center><font style="font-size: 25px;color:green">修改链接</font></center>
		<center><img src="/app/{{ $data->logo }}" id="img-thumbnail" style="width:80px;height:80px"> </center>
<article class="page-container">
	<form  action="/admin/friend_links/{{ $data->id}}" method="post" id="form-admin-add1" class="form form-horizontal" enctype="multipart/form-data" >
		{{ csrf_field() }}
		{{ method_field('PUT') }}
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">链接名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{ $data->links_name }}" placeholder="" id="links_name" name="links_name">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>链接路径：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{ $data->path }}" placeholder="" id="path" name="path">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm/admin-9">
				<input type="file" onchange="upload1()" class="input-text" value="" placeholder="" id="logo" name="logo">
				 <input type="hidden" name="logo-hid" id="form-admin-upload" value="{{ $data->logo }}">
		</div>
			</div>
		</div>

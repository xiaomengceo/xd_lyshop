...
<div class="page-container">
	<form   action="/admin/spec/{{$data->id}}" method="post" class="form form-horizontal" id="form-category-add">
	{{ csrf_field() }}
            ...
            @endif
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-8 col-sm-9">
						<input type="text" class="input-text" value="{{$data->title}}" placeholder="请输入规格名称" id="" name="title" >
					</div>
					<div class="col-3">
					</div>
				</div>
					...
					<div class="formControls col-xs-8 col-sm-9">
					  <table class="table table-border table-bordered">
							...
							<tbody>
								...
								<tr>
								<td><input type="text" class="input-text" value=" {{$v->name}}" placeholder=" " id="" name="name[]" ></td>
								<td><input type="text" class="input-text" value="{{$v->value}}" placeholder=" " id="" name="value[]" ></td>
								<td> 
	...
	$("#form-category-add").validate({
		...
		submitHandler:function(form){
			...
			$(form).ajaxSubmit({
				...
				url: '/admin/spec/{{$data->id}}', // 需要提交的 url
				data:{
                  "_token": "{{ csrf_token() }}",     
			    }, 

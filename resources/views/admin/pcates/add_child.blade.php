...
<div class="page-container">
	<form action="/admin/pcates/store_child" method="post" class="form form-horizontal" id="form-category-add">
	{{ csrf_field() }}
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-8 col-sm-9">
						...
						<select class="select" id="sel_Sub" name="pid" >
							...
							<option @if($cid==$v->id) selected @endif value="{{$v->id}}">{{$v->title}}</option>
							@endforeach 
						</select>
						</span>
					</div>
	...
	$("#form-category-add").validate({
		...
		submitHandler:function(form){
			...
			$(form).ajaxSubmit({
				...
				url: '/admin/pcates/store_child', // 需要提交的 url
				data:{
                  "_token": "{{ csrf_token() }}",     
			    }, 
